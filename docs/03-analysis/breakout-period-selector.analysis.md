...
### 2.7 ScreenerTable.tsx - 정렬 개선 추가 구현

| # | Plan 요구사항 | 구현 상태 | 결과 |
|---|-------------|----------|:----:|
| 1 | 1차 돌파 여부(breakout20=true 우선) -> 2차 기존 sortKey | L1249-1253: `aB = a.conditions.breakout20 ? 1 : 0`, `if (aB !== bB) return bB - aB` | PASS |
| 2 | minPass 기본값 0 -> 5 | L1217: `useState(5)` | PASS |
| 3 | TOP3 추천: 돌파 + A/B등급 종목만 | L1265-1266: `.filter(r => r.conditions.breakout20 && (grade === "A" \|\| grade === "B"))` | PASS |
| 4 | 돌파 종목 좌측 빨간 border 3px | L1488,1495: `isBreakout ? "border-l-[3px] border-l-red-500" : ""` | PASS |
| 5 | 돌파/비돌파 경계에 시각적 구분선 | L1489-1490,1495: `isBoundary = !isBreakout && prevIsBreakout`, `"border-t-2 border-t-gray-300 dark:border-t-gray-600"` | PASS |

---

### 2.8 components/stock-chart-interactive.tsx
...
| ScreenerTable.tsx 파일 크기 | MINOR | 1689줄 (단일 책임 원칙 위반 가능성, 기존 기술부채) |

---

## 9. Summary

**Match Rate: 100%** (65개 검증 항목 중 65개 PASS)

Design v1.0 문서에 기술된 모든 요구사항(60개)과 정렬 개선 추가 구현 요구사항(5개)이 모두 구현에 정확히 반영되었다. 7개 파일에 걸친 period 파라미터화가 일관되게 적용되었으며, 상수 -> 함수 전환 패턴이 Design 명세와 정확히 일치한다.

정렬 개선 4가지 요구사항 구현 요약:
- 정렬: breakout20=true 1차 우선 정렬 + 2차 기존 sortKey 적용
- 필터: minPass 기본값 5로 상향 (고품질 종목 우선 표시)
- TOP3: 돌파 + A/B등급 조건 필터 적용
- 시각: 돌파 종목 빨간 좌측 border + 돌파/비돌파 경계 구분선 (다크모드 대응 포함)
...
1. ScreenerTable.tsx 1689줄 -> 컴포넌트 분리 검토 (ExpertPanel, BuySignalPanel 등 별도 파일)
2. .env.example 파일 생성
...
| 1.1 | 2026-03-07 | 정렬 개선 추가 구현 5항목 추가 (총 65항목) |
